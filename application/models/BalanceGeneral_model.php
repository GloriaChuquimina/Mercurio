...
class BalanceGeneral_model extends CI_Model
{
  ...
  function getGeneralBalanceGeneral($id_entidad,$fecha_inicio,$fecha_fin)
    ...
    //                             ORDER BY nivel ASC,codigo ASC;
		...
		//     return $query->result();
    // }
    function getGeneralBalanceGeneralPorMayorBoliviano($id_entidad,$fecha_inicio,$fecha_fin,$cuenta_mayor,$id_cuenta_mayor,$whereFecha)
	  {
        $query = $this->db_mercurio->query("
                                  SELECT 
                                           pc.id
    ...
    function getGeneralBalanceGeneralPorMayorUSD($id_entidad,$fecha_inicio,$fecha_fin,$cuenta_mayor,$id_cuenta_mayor,$whereFecha)
	  {
        $query = $this->db_mercurio->query("
                                  SELECT 
                                           pc.id
                               ...
                               LEFT JOIN (
                                              SELECT 
                                                      ...
                                                      CASE 
                                                          ...
                                                          THEN 
                                                              (SUM(CASE WHEN dc.tipo_movimiento = 'HB' THEN dc.importe_moneda_extranjera ELSE 0 END) - 
                                                                SUM(CASE WHEN dc.tipo_movimiento = 'DB' THEN dc.importe_moneda_extranjera ELSE 0 END)) * -1
                                                          ELSE 0 
                                                      END AS saldo_cuenta,
                                                      e.id as id_entidad
                                                FROM contabilidad.plancuentas pc
    ...
    function getBalanceGeneralPorMayor($id_entidad,$fecha_inicio,$fecha_fin,$cuenta_mayor,$id_cuenta_mayor,$whereFecha)
	  {
        $query = $this->db_mercurio->query("
                                  SELECT 
                                           pc.id
